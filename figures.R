...
## explained variance ####
tmp <- fread(paste0(path,"z.csv")) %>% apply(1,function(x) names(sort(-table(x)))[1]%>% as.numeric)
z <- vector("numeric",length(foo))
for (i in 1:10) z[tmp==i] <- as.numeric(topicord_eta)[i]
ss <- Y[,-(1:ncovcols)] %>% sapply(function(x) sum((x-mean(x))^2)) %>% sum()

meancast <- topicmeans[,-c(1,length(topicmeans)),with=F]
meancast <- meancast[,match(names(Y[,-(1:ncovcols)]),names(meancast)),with=F]

rss <- (Y[,-(1:ncovcols)]-meancast[z])^2 %>% colSums() 
1-sum(rss)/sum(ss)